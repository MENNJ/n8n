...
export const EditorNameInput = ({ workflowId }: { workflowId: string }) => {
    ...
    useEffect(()=>{
        if(isEditor && inputReff.current){
            inputReff.current.focus()
            inputReff.current.select()
        }
    },[isEditor])
    ...
    return(
        <BreadcrumbItem onClick={()=>setIsEditor(true)} className="cursor-pointer hover:text-foreground transition-colors">
          {workflow.name}
        </BreadcrumbItem>
    )
}
